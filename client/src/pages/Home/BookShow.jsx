...
const BookShow = () => {
  ...
  const { user } = useSelector((state) => state.user); // Extracting user from Redux
  const dispatch = useDispatch(); // Redux dispatch function
  const [show, setShow] = useState(); // State for holding show details
  const [selectedSeats, setSelectedSeats] = useState([]); // State for managing
  // selected seats
  ...
  const getData = async () => {
    try {
      ...
      const response = await getShowById({ showId: params.id }); // API call to fetch
      // show details
      if (response.success) {
          ...
          {Array.from(Array(rows).keys()).map((row) =>
            ...
            Array.from(Array(columns).keys()).map((column) => {
              ...
              if (selectedSeats.includes(seatNumber)) {
                seatClass += " selected"; // Adding 'selected' class if seat is
              }
              if (show.bookedSeats.includes(seatNumber)) {
                seatClass += " booked"; // Adding 'booked' class if seat is already
              }
